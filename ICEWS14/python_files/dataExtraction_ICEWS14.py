...
input_file = anony_path + 'icews_outputs'
# Define the input and output file names
output_file = anony_path + 'ICEWS14/ICEWS_extracted_values.csv'
...
snapshot_pattern = re.compile(r'final_with_new_col_MakeStatement_snapshot_\d{2}_\d{2}\.tsv')
...
for i in range(len(k_l_matches)):
    snapshot_id = int(i/8)
    snapshot_year = snapshot_years[int(i/8)] #if i < len(snapshot_years) else 'N/A'
    print("snapshot_year",snapshot_year)
    k, l = k_l_matches[i]
    AAIL = aail_matches[i]
    AIL = ail_matches[i]
    ...
    if int(k) >= int(l):
        data.append([
            snapshot_id,
            snapshot_year.replace("final_with_new_col_MakeStatement_snapshot_","").replace(".tsv",""),
            k,
            l,
            AAIL,
            AIL,

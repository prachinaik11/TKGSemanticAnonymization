...
directory_path = anony_path + 'ICEWS14/dataset/snapshots_ICEWS14/'
...
for file in files:
    ...
    file_path = f'{anony_path}ICEWS14/dataset/snapshots_ICEWS14/{file}'
    print(file_path)
    ...
    citizenship_df = df[df['predicate'] == '<MakeStatement>'][['subject', 'object']].rename(columns={'object': 'MakeStatement'})

    # Remove the <isCitizenOf> triples from the original DataFrame
    df = df[df['predicate'] != '<MakeStatement>']
    ...
    result_df.to_csv(f'{anony_path}ICEWS14/dataset/snapshots_ICEWS14/withNewCol/with_new_col_MakeStatement_{file}', sep='\t', index=False)
